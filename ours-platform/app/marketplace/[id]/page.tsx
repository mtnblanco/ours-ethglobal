...
export default function PropertyDetailsPage({ params }: { params: { id: string } }) {
  ...
  return (
    <div className="min-h-screen bg-brand-dark text-white pt-24 pb-12 px-6">
      <div className="max-w-7xl mx-auto">
        
        {/* Breadcrumb */}
        <div className="flex items-center gap-2 text-xs text-slate-500 mb-6 uppercase tracking-widest">
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="w-full h-96 bg-slate-800 rounded-2xl border border-white/10 relative overflow-hidden group">
                <div className="absolute inset-0 bg-gradient-to-t from-brand-dark/80 to-transparent" />
                <div className="absolute bottom-8 left-8">
                    <h1 className="text-3xl font-bold mb-2">Skyline Commercial Tower</h1>
            ...
            <div className="min-h-[300px]">
                {activeTab === 'overview' && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6 text-slate-400 leading-relaxed">
                        <p>
                        ...
                        <div className="grid grid-cols-2 gap-4 mt-8">
                            <div className="bg-brand-surface p-4 rounded-lg border border-white/5">
                                <div className="text-xs text-slate-500 uppercase">Asset Value</div>
                                <div className="text-lg font-bold text-white">$125,000,000</div>
                            </div>
                            <div className="bg-brand-surface p-4 rounded-lg border border-white/5">
                                <div className="text-xs text-slate-500 uppercase">Total Tokens</div>
                                <div className="text-lg font-bold text-white">2,500,000 O-SCT</div>
                            </div>
                        </div>
                    </motion.div>
                )}
                     ...
                     <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                        <table className="w-full text-left text-sm text-slate-400">
                            ...
                            <tbody className="divide-y divide-white/5">
                                <tr><td className="px-4 py-3">Gross Rent</td><td className="px-4 py-3 text-right text-white">$8,500,000</td></tr>
                                <tr><td className="px-4 py-3">Operating Expenses</td><td className="px-4 py-3 text-right text-white">($1,200,000)</td></tr>
                                ...
                                <tr><td className="px-4 py-3">Projected Yield</td><td className="px-4 py-3 text-right text-white">12.4%</td></tr>
                            </tbody>
                        </table>
                     </motion.div>
                )}
                ...
                {activeTab === 'documents' && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="grid gap-4">
                        {['Prospectus 2024', 'Property Inspection Report', 'SPV Legal Structure', 'Smart Contract Audit'].map((doc) => (
                            <div key={doc} className="flex items-center justify-between p-4 bg-brand-surface border border-white/5 rounded-lg hover:border-brand-primary/30 transition-colors cursor-pointer group">
                                <div className="flex items-center gap-3">
                                    <div className="p-2 bg-brand-primary/10 text-brand-primary rounded">
                                        <ShieldCheck className="w-5 h-5" />
                                    </div>
                                    <span className="font-medium text-white group-hover:text-brand-primary transition-colors">{doc}</span>
                                </div>
          ...
          <div className="lg:col-span-1">
            <div className="sticky top-24">
                <motion.div 
                    ...
                    className="bg-brand-navy/50 backdrop-blur-xl border border-white/10 rounded-2xl p-6 shadow-2xl shadow-black/50"
                >
                    <div className="flex justify-between items-center mb-6">
                        <h3 className="text-xl font-bold">Buy Tokens</h3>
                        <span className="flex items-center gap-1 text-xs bg-green-500/10 text-green-400 px-2 py-1 rounded border border-green-500/20">
                    ...
                    <div className="flex justify-between items-end mb-6 pb-6 border-b border-white/5">
                        <div>
                            ...
                            <div className="text-3xl font-bold text-white">$50.00 <span className="text-sm font-normal text-slate-500">/ token</span></div>
                        </div>
                        <div className="text-right">
                            <div className="text-sm text-slate-400 mb-1">Min. Buy</div>
                            <div className="text-white font-medium">1 Token</div>
                        </div>
                    </div>
                    ...
                    <div className="space-y-4 mb-6">
                        <div>
                            <label className="flex justify-between text-xs uppercase text-slate-500 font-bold mb-2">
                            ...
                            <div className="relative">
                                <input 
                                    ...
                                    className="w-full bg-brand-dark border border-white/10 rounded-lg px-4 py-3 text-white font-mono focus:border-brand-primary focus:ring-1 focus:ring-brand-primary outline-none transition-all"
                                />
                        ...
                        <div className="flex justify-center">
                             {/* Arrow Separator */}
                             <div className="bg-brand-surface p-1 rounded-full border border-white/10">
                        ...
                        <div>
                            ...
                            <div className="w-full bg-brand-surface border border-white/5 rounded-lg px-4 py-3 text-white font-mono flex justify-between items-center opacity-80">
                    ...
                    <div className="mt-4 text-center">
                        <a href="#" className="text-xs text-slate-500 hover:text-white flex justify-center items-center gap-1 transition-colors">
